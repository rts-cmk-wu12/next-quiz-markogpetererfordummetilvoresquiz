...
<main className="bg-[url('https://static.wixstatic.com/media/c23ee4_8eb76a5d097f4425b3801b5420707127~mv2.gif')] grid-cols-1 grid-rows-1 h-lvh  bg-[cover] px-[10vw] flex flex-col">
    <div className="h-1 bg-blue-500 mt-10"> <div id="progress" className="h-1 bg-amber-300 w-[0%]"></div></div>
        <article className=" bg-[rgb(255,255,255)] mt-10 p-10 ">
            <audio style={{display: "none"}} id="music" src="/set163.m4a" controls></audio>
            {data.results  ? " ": "ups der skete en fejl i api'et....opdater siden : < )"}
        <h1 className="text-center" dangerouslySetInnerHTML={{"__html": data?.results?.[actual]?.question }}></h1>
        <div className="grid grid-rows-2 grid-cols-2 pt-16 gap-11">
         {answers?.map((element) =>{
            return <button value={element}  onClick={(evt)=>{
                checkAnswer(evt)
            }} className="border-2 text-black duration-300 border-amber-300 hover:bg-red-500 hover:text-amber-50 " dangerouslySetInnerHTML={{"__html": element }} key={element}>{}</button>
         })}
         </div>
 ...
 if(actual == data?.results?.length -1 ){
   ...
   function correct(){
        ...
        let words = ["Tillykke", "Sådan mand", "Du kan jo godt", "Fortsæt!!"]
    ...
    function wrong(){
        let gifs = ["https://www.gif-vif.com/gmedia/giphy/epic-fail.gif", "https://25.media.tumblr.com/83ee48c9cb359eff4baafd6196258a0f/tumblr_mfbp9tYQjr1rm90mxo1_400.gif", "https://static.wixstatic.com/media/08906a_0932f10820754116be63e813685cf4e7~mv2.gif", "https://i.chzbgr.com/full/9281817344/h20249185/nope", "https://i.chzbgr.com/full/5530818560/hA1F25FC9/watch-me-fail-mommy", "https://media.tenor.com/vauuoXz1mgoAAAAM/epic-fail-falling.gif", "https://cdn.webfail.com/upl/img/921fc9fd031/post2.jpg"]
